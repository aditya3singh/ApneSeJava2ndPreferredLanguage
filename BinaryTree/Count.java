...
public class Main {
  ...
  public static int heightoTree(Node root){
    if(root == null) return 0;
    int lh = heightoTree(root.left);
    int rh = heightoTree(root.right);
    
    return Math.max(lh, rh) + 1;
    
  }
  
  public static int countTreeN(Node root){
    if(root == null) return 0;
    
    int ln = countTreeN(root.left);
    int rn = countTreeN(root.right);
    return ln + rn + 1; 
  }
